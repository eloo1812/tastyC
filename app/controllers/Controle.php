...
class Controle extends Base {
    ...
    public function store($request, $response, $args) {
        ...
        $created = $this->comprados->create([
            ...
            'valor' => 0 // Defina o valor conforme necessário
        ]);
    ...
    public function destroy($request, $response, $args)
    {
        ...
        $comprado = $this->comprados->findBy('id', $id);
        ...
        $deleted = $this->comprados->delete('id', $id);

        if ($deleted)
        {
            Flash::set('message', 'Comprado com sucesso!');
            return \app\helpers\redirect($response, '/home');
        }

        Flash::set('message', 'Não foi possível comprar', 'danger');
        return \app\helpers\redirect($response, '/home');
}
}